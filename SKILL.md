...
2. Select the role explicitly requested by the user or assigned in the incoming task prompt. If no role is assigned, use Context Agent when background is missing; otherwise propose Commander. State the selected role. Keep that role across turns until the user explicitly changes it; project phase changes and task completion do not change conversation identity.
...
- **Prompts**: every startup prompt is independently copyable with task ID, target window, absolute must-read/output paths, scope, dependencies, effective permissions, gates, acceptance criteria, and relevant backup/human prerequisites. A reply may contain several independent startup prompts. For an existing task use `templates/task-continuation.md`; preserve task identity and accepted evidence across turns.
